function [failPercentage, thoughtValidPercentage, nfbVector] = computePerformanceA(parameterVector)
...
for i=1:totalScanned
   ...
   [serialNumber, isValid, theReason, numberOfBars] = decodeFred(theSignal, energyThreshold, codeTreshold, growFilterWidth);
   totalThoughtValid = totalThoughtValid + isValid;
   nfbVector(i) = numberOfBars;
   if(isValid)
       %serialNumber
   else
       reasonCounter(theReason) = reasonCounter(theReason) + 1;
   end
...
fprintf('Le pourcentage de détections erronées est de %f pourcents. Bravo.\n', fuckupPercentage);
fprintf('Le pourcentage de réussite pensé par l''algorithme est de %f pourcents. Bravo.\n', thoughtValidPercentage);
%reasonCounter'
end
