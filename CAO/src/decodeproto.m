function [ NumeroSerie, valide ] = decodeproto(tableau)
...
%filtreSegmentation = [-1 0 1];
%grad = conv(tableau, filtreSegmentation);
%absGrad = abs(grad);
%plot(absGrad)
absGrad = abs(gradient(tableau));
% figure;
plot(absGrad)
vecSeuil = absGrad > 150;
% vecSeuil(end) = 0;
% vecSeuil(end-1) = 0;
vecSeuilInverse = flipud(vecSeuil);
longueurGradAbs = length(vecSeuil);
%plot(absGrad)
i = 3;
switchState = 0;
switchStatei = 0;
...
fin=1;
buffs=0; buffe=0;
while i < longueurGradAbs
    
    if vecSeuil(i)~=vecSeuil(i+1)
        switchState = switchState + 1;
    end
    
    if vecSeuilInverse(i)~=vecSeuilInverse(i+1)
        switchStatei = switchStatei + 1;
    end
    
    if (switchState==1 && buffs==0)
        start = i+1;
        buffs = 1;
    end
    
    if (switchStatei==7 && buffe==0)
        fin = longueurGradAbs - i;
        buffe = 1;
    end
...
vecSegmenter = tableau(start:fin);
longueurVecSegmenter = length(vecSegmenter);
%figure;
%plot(vecSegmenter)
%% detrend
vecD=detrend(vecSegmenter(1:longueurVecSegmenter));
%figure;
%plot(vecD)
%veci=flipud(vecD);
len=size(vecD,1);
...
max=500; min=490; %doit être supérieur au bruit
...
if verif==1
    ...
    %display('Une erreur est survenue. Le code nest pas valide')
    noserie=serie;
    valide = 0;
end

vecPuissance = flipud((0:11)');
vecDecalage = 10.^vecPuissance;
NumeroSerie = uint64(sum(noserie.*vecDecalage));
end
